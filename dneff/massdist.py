...
from src.integrator import Simp1D # Our 1D integrator
...
def FBEqs(x, v, rPBHi, rRadi, nPBHi, nphi, Mi, asi, sDR, fP_fBH):
    ...
    H   = sqrt(8 * pi * bh.GN * (rPBH * 10.**(-3*x) + rRad * 10.**(-4*x))/3.) # Hubble parameter
    ...
    dTdx    = - (Tp/Del) * (1.0 - (bh.gstar(Tp)/bh.gstarS(Tp))*(0.25*drRaddx/(rRad+rDRD)))
...
class FBEqs_Sol:
   ...
   #----------------------------------------------------------------------------------------------------------------------------------#
    ...
    def Solt(self):
        ...
        ti     = (sqrt(45./(16.*pi**3.*bh.gstar(Ti)*bh.GN))*Ti**-2) # Initial time, assuming a radiation dom Universe
        ...
        fM = interpolate.interp1d(10.**t_solg/10.**tau_solg, M_solg/Mb, bounds_error=False, fill_value=(M_solg[0]/Mb, M_solg[-1]/Mb))
        ...
        ftau = interpolate.CubicSpline(Mit1, tau_PBH, extrapolate=True)
        ...
        if bi > 1.e-19*(1.e9/Mi):
            ...
            xfw = sqrt(1. + 4.*10.**tfn*sqrt(2.*pi*bh.GN*rRadi/3.))
            xflog10 = log10(xfw)
