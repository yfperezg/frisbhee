...
def sigma_v(T1, T2, paramsDM):
    ...
    integ = Simp1D(Int, pars, lims).integral()
...
def FBEqs(x, v, nphi, paramsDM, GammaX, p_DM, p_X, Br_DM, FO):
    ...
    GXt = min([GammaX * kn(1, zBH)/kn(2, zBH), 1e5*H])
    p_X_0  = p_X/a  
    ...
    NDMeq = (a**(3) * mDM**2 * Tp * kn(2,z))/(pi**2)/nphi
...
def FBEqs_aBE(x, v, nphi, paramsDM, GammaX, x_evap, T_bh_in, p_DM, p_X, Br_DM, FO):
    ...
    ####
    p_X_0  = p_X  * 10.**(x_evap - x)
    p_DM_0 = p_DM * 10.**(x_evap - x)
    ...
    NDMeq = (a**(3) * mDM**2 * Tp * kn(2, z))/(pi**2)/nphi # Equilibrium DM number density
    ...
    dNDMBdx = 2*(Br_DM*GXt/H)*NX

    dNXdx   = -NX*GXt/H
    
    dNDMCdx = -(NDMC**2 -  NDMeq**2)*svTT*nphi/(H*a**(3))  # Thermal Contribution w/o PBH evap
    
    dNDMHdx = 2*(Br_DM*GXt/H)*NX                              # PBH-induced contribution w/o contact
    
    dEqsdx  = [drRaddx, dTdx, dNDMTdx, dNDMBdx, dNXdx, dNDMCdx, dNDMHdx]
...
#------------------------------------------------------------------------------------------------------------------------------------#
    ...
    def Omegah2(self):
        ...
        MPL = lambda t, x:StopMass(t, x, Mi)
        MPL.terminal  = True
        MPL.direction = -1.
        ...
        if bi > 1.e-19*(1.e9/Mi):
            xf = root(bh.afin, [40.], args = (rPBHi, rRadi, 10.**tau, 0.), method='lm', tol=1.e-10) # Scale factor 
            xflog10 = xf.x[0]            
        else:
        ...
        solFBE_aBE = solve_ivp(lambda t, z: FBEqs_aBE(t, z, nphi, paramsDM, G_X, xflog10, bh.TBH(solFBE.y[0,-1],solFBE.y[1,-1]),
                                                      p_DM, p_X, BR, FO),
                               [xflog10, xfmax], v0aBE, method='Radau', rtol=1.e-7, atol=1.e-10)
        ...
        x    = np.concatenate((solFBE.t[:], solFBE_aBE.t[:]), axis=None)
        ...
        H   = np.sqrt(8 * pi * bh.GN * (PBH * 10.**(-3*x) + Rad * 10.**(-4*x))/3.) # Hubble parameter
        npt = T.shape[0]
        TBH = np.concatenate((bh.TBH(solFBE.y[0,:],asi), np.sqrt(mDM**2 + p_DM**2*10.**(2.*(xflog10 - solFBE_aBE.t[:])))), axis=None)
        ...
        Oh2   =  (NDMT + NDMB) * nphi * 10.**(-3.*x) * mDM * Conf
        Oh2Th =  NDMT * nphi * 10.**(-3.*x) * mDM * Conf
        Oh2BH =  NDMB * nphi * 10.**(-3.*x) * mDM * Conf

        Oh2C =  NDMC * nphi * 10.**(-3.*x) * mDM * Conf # DM density w/o PBH contribution
        
        a0= 10**x[-1] * (T[-1]/T0)*(bh.gstarS(T[-1])/bh.gstarS(T0))**(1./3.)
